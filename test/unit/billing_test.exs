defmodule Report.BillingTest do
  ...
  describe "Billings API" do
    ...
    test "billing_changeset/5" do
      ...
      billing =
        Billings.billing_changeset(
          %Billing{},
          declaration,
          declaration.legal_entity,
          declaration.person,
    ...
    test "billing_logs_changeset/5" do
      declaration = make_declaration_with_all() |> Repo.preload([:legal_entity, :person, :division])
      Repo.insert(Billings.billing_changeset(%Billing{}, declaration, nil, declaration.person, declaration.division))
      assert length(Repo.all(Report.BillingLog)) == 1

      Repo.insert(
        Billings.billing_changeset(%Billing{}, declaration, declaration.legal_entity, nil, declaration.division)
      )
